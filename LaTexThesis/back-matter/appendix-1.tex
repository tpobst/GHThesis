%try to get all of the titles on the same page as the table or graph
\chapter{}

	\section{Site Data}
		\input{FiguresnTables/SiteData1}

	\section{Site data}
		\input{FiguresnTables/SiteData2}


\chapter{Descriptive Statistics}
	\input{FiguresnTables/DescriptiveStats}


\chapter{Variable selection}
	\input{FiguresnTables/variables}


\chapter{Julian Date Coefficients}
	\section{Step-wise Method}
		 \input{FiguresnTables/SWjuliandate}

	\section{Temporal Variables}
		\input{FiguresnTables/TVjuliandate}%need to clean up


\chapter{ANOVA/Bonferoni}\pagebreak
	\section{pH}
		\input{FiguresnTables/ANOVApH}
 
	\section{ANC}%decreasing ANC trends in Bonferoni
		\input{FiguresnTables/ANOVAANC}

	\section{Nitrate}
		\input{FiguresnTables/ANOVANitrate}

	\section{Sulfate}
		\input{FiguresnTables/ANOVASulfate}


\chapter{Post Hoc Power Analsysis}
	\section{Step-Wise Variables}
		\input{FiguresnTables/SWposthoc}%needs cleaning


	\section{Temperol variables}
		\input{FiguresnTables/TVposthoc}%needs cleaning


\chapter{A priori analysis}
	\section{Power graphs}
		\subsection{pH}
			\input{FiguresnTables/pHPowerGraph}%maybe transfer to table notation instead of coordinates
		\subsection{ANC and Nitrate}
			\input{FiguresnTables/ANCnNitratepowergraph}
		\subsection{Sulfate}
			\input{FiguresnTables/Sulfatepowergraph}
		\subsection{Time Variables}
			\input{FiguresnTables/TVpowergraph}



















